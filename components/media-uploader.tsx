...
import { RiCloseLargeLine, RiUploadCloudLine } from "@remixicon/react";
...
const variants = {
  base: "relative my-2 rounded-md flex justify-center items-center flex-col cursor-pointer min-h-[150px] min-w-[200px] border border-dashed border-gray-400 dark:border-gray-300 transition-colors duration-200 ease-in-out",
  image:
...
const MultiMediaDropDown = React.forwardRef<HTMLInputElement, InputProps>(
  (
              ...
              {imageUrls[index] && !disabled && progress === "PENDING" && (
                ...
                >
                  <div className="flex h-5 w-5 cursor-pointer items-center justify-center rounded-md border border-solid border-gray-500 bg-white transition-all duration-300 hover:h-6 hover:w-6 dark:border-gray-400 dark:bg-black">
                    <RiCloseLargeLine className="w-4 h-4 text-gray-500 dark:text-gray-400" />
                  </div>
                </div>
              )}
            </div>
...
MultiMediaDropDown.displayName = "MultiMediaDropDown";

export { MultiMediaDropDown };
